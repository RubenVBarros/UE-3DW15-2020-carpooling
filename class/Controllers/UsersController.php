...
class UsersController
{
    ...
    public function createUser(): string
    {
        ...
        if (isset($_POST['firstname']) &&
            ...
            //Create the user cars relation
            $isOk = true;
            if(!empty($_POST['cars']))
            {
                foreach($_POST['cars'] as $idCar)
                {
                    $isOk = $usersService->setUserCars($idUser,$idCar);
                }
            }
            if ($idUser && $isOk) {
    ...
    public function getUsers(): string
    {
        ...
        foreach ($users as $user) {
            ...
            if(!empty($user->getCars()))
            {
                ...
                {
                    $cars .= $car->getBrand() . ' ' . $car->getModel() . ' ' . $car->getColor() . ' ';
                }
            }
            $html .=
                '#' . $user->getId() . ' ' .
